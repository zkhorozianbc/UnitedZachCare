...
def submit_coverage_request(policy_name, row) -> dict[str, Any]:
    with Session.begin() as session:
        medical_condition = get_or_create_model(session, MedicalCondition, name=row["medical_history"])
        family_medical_condition = get_or_create_model(session, MedicalCondition, name=row["family_medical_history"])
...
def update_medical_history(customer_id: int, new_medical_data: dict[str, Any]) -> int:
    with Session.begin() as session:
        medical_history = session.query(MedicalHistory).filter_by(customer_id=customer_id).first()
        medical_condition_attrs = {"medical_condition", "family_medical_condition"}
        medical_history_fields = set(medical_history.as_dict().keys()).union(medical_condition_attrs)
        if set(new_medical_data) - medical_history_fields:
            raise ValueError(f"Unknown medical history fields provided: {new_medical_data}")
        for attr, value in new_medical_data.items():
            if attr in medical_condition_attrs:
                medical_condition = get_or_create_model(session, MedicalCondition, name=new_medical_data[attr])
        ...
        policy_instance = get_or_create_model(session, PolicyInstance, customer_id=customer_id)
